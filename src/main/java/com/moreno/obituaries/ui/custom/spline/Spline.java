package com.moreno.obituaries.ui.custom.spline;
...
public class Spline {
    ...
    public List<SplinePoint> createSpline(int numberItems,float f, SplinePoint... point) {
        this.points = point;
        List<SplinePoint> list = new ArrayList<>();
        for (float t = 0f; t < (point.length - 3.0f) * f; t += 0.01f) {
            list.add(getSpline(numberItems,t));
        }
        return list;
    }

    public SplinePoint getSpline(int numberItems,float t) {
        int p0, p1, p2, p3;
        p1 = (int) t + 1;
        p2 = p1 + 1;
        if(numberItems==1){
