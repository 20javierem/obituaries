package com.moreno.obituaries.ui.custom.chart;

import com.moreno.obituaries.ui.custom.chart.blankchart.BlankPlotChart;
import com.moreno.obituaries.ui.custom.chart.blankchart.BlankPlotChatRender;
import com.moreno.obituaries.ui.custom.spline.Spline;
import com.moreno.obituaries.ui.custom.spline.SplinePoint;
...
public class CurveLineChart extends JComponent {
    ...
    private void createChart() {
        blankPlotChart.setBlankPlotChatRender(new BlankPlotChatRender() {
            ...
            public void renderGraphics(BlankPlotChart chart, Graphics2D g2, Rectangle2D rectangle) {
                if (!model.isEmpty() && animate > 0 && index >= 0 && index <= legendItems.size() - 1) {
    ...
    private void draw(Graphics2D g2, Rectangle2D rec, int index, double maxValue) {
        ...
        g2.setColor(legendItems.get(index).getLegend().getColor1());
        current = copyPoint(points);
        List<SplinePoint> list = spline.createSpline(model.size(), animate, points);
        ...
        g2.draw(path);
        if (currentPoint != -1) {
            SplinePoint s = spline.getSpline(model.size(), currentPoint);
            drawLabel(g2, s);
        }
    }
    ...
    public void addLegend(String name, Color color1, Color color2) {
        ...
        legend.addActionListener(e -> action(legend));
        if (legendItems.size() - 1 == index) {
    ...
    private void action(LegendItem legend) {
        if (animate > 0) {
            startChange(legendItems.indexOf(legend));
    ...
    public void clear() {
        ...
        model.clear();
        legendItems.forEach(legendItem -> panelLegend.remove(legendItem));
        legendItems.clear();
        selectedIndex = -1;
        alphaLable = 0f;
        currentPoint = 0f;
    ...
    public void start() {
        if (!animator.isRunning() && !model.isEmpty()) {
            index = legendItems.size() - 1;
            clearLegendSelected(legendItems.get(index));
            color1 = legendItems.get(index).getLegend().getColor1();
            color2 = legendItems.get(index).getLegend().getColor2();
    ...
    private void startChange(int index) {
        if (this.index != index) {
            ...
            timingColor1 = new PropertySetter(this, "color1", color1, legendItems.get(index).getLegend().getColor1());
            timingColor2 = new PropertySetter(this, "color2", color2, legendItems.get(index).getLegend().getColor2());
